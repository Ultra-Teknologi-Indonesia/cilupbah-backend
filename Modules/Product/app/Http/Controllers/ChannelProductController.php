...
class ChannelProductController extends Controller
{
    ...
    public function bulkUnlink(Request $request): JsonResponse
    {
        $items = $request->validate([
            ...
            'items.*.channel' => ['required', 'string', 'max:50'],
            'items.*.external_product_id' => ['required', 'string', 'max:255'],
            'items.*.shop_id' => ['required', 'string', 'max:255'],
        ])['items'];

        $results = [];
        foreach ($items as $item) {
            try {
                $this->channelProductService->unlinkProduct(
                    $item['external_product_id'],
                    $item['shop_id'],
                );
                $results[] = ['external_product_id' => $item['external_product_id'], 'status' => 'success'];
            } catch (\Throwable $e) {
                $results[] = [
        ...
        return $this->successResponse([
            ...
            'results' => $results,
        ], 'Pemutusan koneksi channel diproses secara massal.');
    }
