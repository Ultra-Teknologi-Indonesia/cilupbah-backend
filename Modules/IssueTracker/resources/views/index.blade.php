                ...
                <tr class="hover:bg-gray-50/50 cursor-pointer" onclick="window.location='{{ route('issues.show', $issue) }}'">
                    ...
                    <td class="px-3 py-3 text-right" onclick="event.stopPropagation()">
                        <form method="POST" action="{{ route('issues.destroy', $issue) }}"
                              onsubmit="return confirm('Yakin hapus issue &quot;{{ addslashes($issue->title) }}&quot;? Tidak bisa dikembalikan.');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" title="Hapus" class="rounded-lg p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M9.5 3h5a1 1 0 011 1v2H8.5V4a1 1 0 011-1z"/></svg>
                            </button>
                        </form>
                    </td>
                </tr>
