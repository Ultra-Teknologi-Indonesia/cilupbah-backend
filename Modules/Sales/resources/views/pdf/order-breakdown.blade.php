@php
    ...
    $biayaLainnya  = (float) ($order->service_fee ?? 0) + (float) ($order->transaction_fee ?? 0) + (float) ($order->order_processing_fee ?? 0);
    $diskonOngkir  = (float) ($order->platform_shipping_rebate ?? 0);

    $isManual = (bool) ($order->is_manual ?? false);
    $paidTotal = $isManual && $order->relationLoaded('invoices')
        ? (float) $order->invoices->sum(fn ($inv) => (float) $inv->paid_amount)
        : 0.0;
    $sisaPembayaran = max($grandTotal - $paidTotal, 0.0);

    $isSettled = (bool) $order->is_settled;
    $settlementAmount = $order->settlement_amount !== null ? (float) $order->settlement_amount : null;
    $refundTotal = $order->relationLoaded('returns')
        ? (float) $order->returns->sum(fn ($r) => (float) ($r->settlement->total_amount ?? 0))
        : 0.0;
@endphp
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Rincian Pesanan {{ $order->salesorder_no }}</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .items-table .desc { max-width: 45mm; word-wrap: break-word; }
        ...
        .income-table td { padding: 0.8mm 0; font-size: 8.5pt; }
        .income-table .label { color: #555; }
        .income-table .value { text-align: right; font-family: DejaVu Sans Mono, monospace; }
    ...
    <table class="header-table">
        <tr>
            <td>
                ...
                <div class="header-subtitle">Faktur &amp; Rincian Penghasilan</div>
            </td>
            <td>
...
<table class="items-table">
    <thead>
        <tr>
            ...
            <th class="text-center" style="width:10mm;">DISK%</th>
            <th class="text-right">Nilai Diskon</th>
            <th class="text-center" style="width:10mm;">PAJAK%</th>
    ...
    <tbody>
        @foreach($order->items as $i => $item)
            @php
                $itemBase = ((float) $item->price * (int) $item->qty_in_base) - (float) $item->disc_amount;
                $itemTaxPct = $itemBase > 0 && (float) $item->tax_amount > 0
                    ? ((float) $item->tax_amount / $itemBase) * 100
                    : 0.0;
            @endphp
            <tr>
                ...
                <td class="text-center">{{ $itemTaxPct > 0 ? number_format($itemTaxPct, 0) . '%' : '-' }}</td>
    ...
    <table class="summary-table">
        ...
        @if($isManual)
            <tr>
                ...
                <td class="value">{{ number_format($paidTotal, 0, ',', '.') }}</td>
            </tr>
            <tr>
                <td class="label">Sisa Pembayaran</td>
                <td class="value">{{ number_format($sisaPembayaran, 0, ',', '.') }}</td>
            </tr>
        @endif
        <tr class="status-row">
...
@if($isSettled)
    <div class="income-section">
        <div class="income-title">Rincian Penghasilan</div>
        <table class="income-table">
            <tr>
                <td class="label">Harga Produk</td>
                <td class="value">{{ number_format($subTotal, 0, ',', '.') }}</td>
            </tr>
            @if($refundTotal > 0)
                <tr>
                    <td class="label">Jumlah Pengembalian Dana</td>
                    <td class="value negative">-{{ number_format($refundTotal, 0, ',', '.') }}</td>
                </tr>
            @endif
            @if($shippingCost > 0)
                <tr>
                    <td class="label">Ongkir Dibayar Pembeli</td>
                    <td class="value">{{ number_format($shippingCost, 0, ',', '.') }}</td>
                </tr>
            @endif
            @if($order->seller_shipping_borne)
                <tr>
                    <td class="label">Ongkir Dibayarkan ke Jasa Kirim</td>
                    <td class="value negative">-{{ number_format((float) $order->seller_shipping_borne, 0, ',', '.') }}</td>
                </tr>
            @endif
            <tr class="total-row">
                <td class="label">Total Penghasilan</td>
                <td class="value {{ ($settlementAmount ?? 0) < 0 ? 'negative' : '' }}">
                    {{ ($settlementAmount ?? 0) < 0 ? '-' : '' }}Rp {{ number_format(abs($settlementAmount ?? 0), 0, ',', '.') }}
                </td>
            </tr>
        </table>
    </div>
