...
class InventoryService
{
    ...
    {
        return $this->adjustInternal($data, allowInboundBin: false);
    }

    /**
     * Apply a stock delta generated by the inbound receiving lifecycle.
     *
     * Inbound/DEFAULT is intentionally excluded from generic stock operations,
     * but receiving, receiving corrections, and their reversals must mutate
     * that staging bin before putaway.
     */
    public function adjustInboundStaging(array $data): Inventory
    {
        return $this->adjustInternal($data, allowInboundBin: true);
    }
    ...
    private function adjustInternal(array $data, bool $allowInboundBin): Inventory
    {
        ...
        if ($allowInboundBin) {
            app(InboundBinPolicy::class)->assertInbound(
                $data['location_id'],
                $data['bin_id'] ?? null,
                'penerimaan stok',
            );
        } elseif (! empty($data['bin_id'])) {
        ...
        $transfer = DB::transaction(function () use ($transferId, $data) {
            ...
            foreach ($transfer->items as $item) {
                ...
                if ($transitInventory) {
                    ...
                    $rejectDeduct = min((int) $rejectedQty, (int) $transitInventory->on_hand);
                    if ($rejectDeduct > 0) {
                        $transitInventory->on_hand -= $rejectDeduct;
                        $this->inventoryRepository->updateStock($transitInventory);
                        ...
                        $sourceInventory->on_hand += $rejectDeduct;
                        $this->inventoryRepository->updateStock($sourceInventory);
